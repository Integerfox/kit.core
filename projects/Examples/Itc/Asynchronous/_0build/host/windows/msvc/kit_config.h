#ifndef KIT_CONFIG_H_
#define KIT_CONFIG_H_
/*-----------------------------------------------------------------------------
 * Copyright Integer Fox Authors
 *
 * Distributed under the BSD 3 Clause License. See the license agreement at:
 * https://github.com/Integerfox/kit.core/blob/main/LICENSE
 *
 * Redistributions of the source code must retain the above copyright notice.
 *----------------------------------------------------------------------------*/
/** @file Project/build specific 'Options' (see LConfig Pattern) */

#define USE_KIT_SYSTEM_TRACE
#define USE_KIT_SYSTEM_ASSERT

#define OPTION_EXAMPLES_DM_TRANSACTION_CLIENT_NUM_ITERATIONS 10

// NOT using Data Model change notifications in the Event Queue - only ITC message notifications
#define USE_KIT_EVENT_QUEUE_ITC_MSG_ONLY

#endif
