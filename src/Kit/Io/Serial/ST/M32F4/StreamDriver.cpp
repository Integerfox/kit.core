...
StreamDriver::HalMapping_T Kit::Io::Serial::ST::M32F4::StreamDriver::m_mappings[OPTION_KIT_IO_SERIAL_ST_M32F4_MAX_UARTS];

#define ENTER_CRITICAL_SECTION()              \
    do                                        \
    {                                         \
        Kit::System::suspendScheduling();     \
        Bsp_NVIC_disable_irq( m_uartIrqNum ); \
    }                                         \
    while ( 0 )
#define EXIT_CRITICAL_SECTION()              \
    do                                       \
    {                                        \
        Bsp_NVIC_enable_irq( m_uartIrqNum ); \
        Kit::System::resumeScheduling();     \
    }                                        \
    while ( 0 )

////////////////////////
StreamDriver::StreamDriver( Kit::Container::RingBuffer<uint8_t>& txBuffer,
    ...
    , m_errCount( 0 )
{
    ...
    bool mapped = false;
    for ( unsigned idx = 0; idx < OPTION_KIT_IO_SERIAL_ST_M32F4_MAX_UARTS; idx++ )
    {
    ...
    if ( !mapped )
    {
        Kit::System::FatalError::logf( Kit::System::Shutdown::eDRIVER, "Kit::Io::Serial::ST::M32F4::StreamDriver. Too many driver instances." );
    }
}
