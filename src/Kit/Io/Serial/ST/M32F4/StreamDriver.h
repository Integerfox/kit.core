#ifndef KIT_IO_SERIAL_ST_M32F4_STREAMDRIVER_H_
#define KIT_IO_SERIAL_ST_M32F4_STREAMDRIVER_H_
/*------------------------------------------------------------------------------
...
#ifndef OPTION_KIT_IO_SERIAL_ST_M32F4_MAX_UARTS
#define OPTION_KIT_IO_SERIAL_ST_M32F4_MAX_UARTS 1
#endif
...
public:
    /** Constructor. Note: The driver does not begin operating until start() is
        called.
     */
    StreamDriver( Kit::Container::RingBuffer<uint8_t>& txBuffer,
                  Kit::Container::RingBuffer<uint8_t>& rxBuffer ) noexcept;
...
protected:
    ...
    Kit::System::Thread* m_txWaiterPtr;

    /// Handle of the blocked RX client thread (if there is one)
    Kit::System::Thread* m_rxWaiterPtr;

    /// IRQ number of the UART being used
    IRQn_Type m_uartIrqNum;

    /// Transmit buffer
    Kit::Container::RingBuffer<uint8_t>& m_txBuffer;

    /// Receive buffer
    Kit::Container::RingBuffer<uint8_t>& m_rxBuffer;
    ...
    static HalMapping_T m_mappings[OPTION_KIT_IO_SERIAL_ST_M32F4_MAX_UARTS];
};
