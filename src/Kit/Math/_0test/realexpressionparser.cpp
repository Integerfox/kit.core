...
#include "Kit/Math/RealExpressionParser.h"
...
static void compare( RealExpressionParser<T>& parser, T expectedResult, const char* str )
{
    ...
    KIT_SYSTEM_TRACE_MSG( SECT_, "expr=%s, expected=%g, result=%g",
                                   str,
                                   (double) expectedResult,
                                   (double) result) ;
    REQUIRE( success == true );
    REQUIRE( Kit::Math::almostEquals<T>( expectedResult, result, KIT_MATH_REAL_FLOAT_EPSILON ) );
}
...
TEST_CASE( "realexpressionparser" )
{
    ...
    SECTION( "float" )
    {
        KIT_SYSTEM_TRACE_MSG( SECT_, ("float") );
    ...
    SECTION( "double" )
    {
        KIT_SYSTEM_TRACE_MSG( SECT_, ("double") );
