#ifndef KIT_CONTAINER_AVLTREE_H_
#define KIT_CONTAINER_AVLTREE_H_
/*------------------------------------------------------------------------------
 ...
 */
#ifndef OPTION_KIT_CONTAINER_AVLTREE_MAX_TREE_DEPTH
#define OPTION_KIT_CONTAINER_AVLTREE_MAX_TREE_DEPTH      32
#endif
...
private:
    ...
    class AvlRoot : public MapItem, public Key
    {
    public:
        /// 
    ...
    public:
        ...
        /// Special constructor
        AvlRoot( const char* ignoreThisParameter_usedToCreateAUniqueConstructor ) noexcept;
...
public:
    ...
    /** Deletes the specified item from the tree.  Returns the node deleted.  
        If the delete fails (i.e. node does not exist in the tree), then nullptr
        is returned.
     */
    MapItem* removeItem( MapItem& node );

    /// Searches for a item with a matching key.  Returns nullptr if not found
    MapItem* find( const Key& keyToFind ) const;

    /// Returns the first item in the tree. Returns nullptr if tree is empty
    MapItem* first() const;

    /// Returns the last item in the tree. Returns nullptr if tree is empty
    MapItem* last() const;

    /// Returns the next item in the tree.  Returns nullptr if at the end-of-tree
    MapItem* next( MapItem& current ) const;

    /// Returns the previous item in the tree.  Returns nullptr if at the start-of-tree
    MapItem* previous( MapItem& current ) const;
