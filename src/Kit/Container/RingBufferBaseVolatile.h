#ifndef KIT_CONTAINER_RINGBUFFERBASEVOLATILE_H_
#define KIT_CONTAINER_RINGBUFFERBASEVOLATILE_H_
/*------------------------------------------------------------------------------
...
/** This 'implementation' class provides the base functionality for a ring buffer.
    ...
    NOTE: This class relies on the 'volatile' qualifier to ensure that the
          read/write indexes are properly updated across threads/ISRs.  It should
          ONLY be used with hardware targets that DO NOT support std::atomic with
          full lock-free semantics.  For example - the Arm Cortex-M0 does NOT
          have hardware instructions to support full lock-free atomics.
 ...
 */
class RingBufferBaseVolatile
{
protected:
    /// Constructor
    RingBufferBaseVolatile( unsigned numMaxElements ) noexcept;
...
private:
    ...
    RingBufferBaseVolatile( const RingBufferBaseVolatile& m ) = delete;

    /// Prevent access to the assignment operator -->Ring Buffers can not be copied!
    RingBufferBaseVolatile& operator=( const RingBufferBaseVolatile& m ) = delete;

    /// Prevent access to the move constructor -->Ring Buffers can not be implicitly moved!
    RingBufferBaseVolatile( RingBufferBaseVolatile&& m ) = delete;

    /// Prevent access to the move assignment operator -->Ring Buffers can not be implicitly moved!
    RingBufferBaseVolatile& operator=( RingBufferBaseVolatile&& m ) = delete;
};
