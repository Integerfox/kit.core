...
/// clang-format off
/// Reflected table driven, aka fast CRC calculation
...
static constexpr uint32_t RESIDUE_ = 0xDEBB20E3;
...
void Crc32EthernetFast::accumulate( const void* bytes, unsigned numbytes ) noexcept
{
    ...
    {
        m_crc = table_[( m_crc ^ *ptr ) & 0xFFU] ^ ( m_crc >> 8 );
    }
}

bool Crc32EthernetFast::finalize( void* destBuffer, unsigned destBufferSize ) noexcept
{
