...
protected:
    void entry() noexcept override
    {
        ...
        KIT_SYSTEM_TRACE_MSG( SECT_, "Thread Running: %s, %p", t1.getName(), (void*)( (size_t)( t1.getId() ) ) );
        ...
        for ( ;; )
        {
            ...
            if ( now - timeMark2 >= 1000 )
            {
                Bsp_toggle_debug2();
                timeMark2 = now;
                KIT_SYSTEM_TRACE_MSG( SECT_, "now=%" PRIu32, now );
            }
...
int main( void )
{
    ...
    KIT_SYSTEM_TRACE_MSG( SECT_, "**** BSP TEST APPLICATION STARTED ****" );

    //// Create my test thread
    Kit::System::Thread* t1 = Kit::System::BareMetal::Thread::create( runnable_, "TEST", KIT_SYSTEM_THREAD_PRIORITY_NORMAL );
    if ( t1 == nullptr )
    {
    ...
    while ( 1 )
        ;  // Enable scheduling should NEVER return!
}
