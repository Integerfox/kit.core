...
///////////////////////////////
TextBlock::TextBlock( char* string,
                      char  delimiter,
                      char  terminator,
                      char  quote,
                      char  escape )
    : m_base( (char*) Strip::space( string ) )
    , m_ptr( m_base )
    , m_count( 0 )
    , m_validTokens( true )
    , m_terminatorFound( false )
{
    ...
    // Housekeeping
    char* firstNonSpacePtr = nullptr;
    char* lastNonSpacePtr  = nullptr;
    ...
    while ( *m_ptr != '\0' )
    {
        ...
        if ( *m_ptr == delimiter )
        {
            ...
            *m_ptr++ = '\0';
            ...
            // Housekeeping
            firstNonSpacePtr = nullptr;
            lastNonSpacePtr  = nullptr;
        ...
        else
        {
            ...
            else if ( lastNonSpacePtr != nullptr )
            {
                if ( !isspace( (int)*m_ptr ) )
                {
                    lastNonSpacePtr = m_ptr;
