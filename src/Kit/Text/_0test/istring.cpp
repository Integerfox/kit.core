...
static void vformat( IString& s, const char* format, ... )
{
    va_list ap;
    va_start( ap, format );
    s.vformat( format, ap );
...
static void vformatAppend( IString& s, const char* format, ... )
{
    va_list ap;
    va_start( ap, format );
    s.vformatAppend( format, ap );
    va_end( ap );
}

static void vformatOpt( bool flag, IString& s, const char* format, ... )
{
    va_list ap;
    va_start( ap, format );
    s.vformatOpt( flag, format, ap );
    va_end( ap );
}
...
TEST_CASE( "IString" )
{
    ...
    FString<31> bar( "Hello World, this is bob" );
    IString&    fref = foo;
    IString&    bref = bar;
    ...
    SECTION( "Some basic operation" )
    {
        ...
        bar.format( "Hello World" );
        REQUIRE( bar == "Hello World" );
        bar.formatAppend( "XXX" );
        REQUIRE( bar == "Hello WorldXXX" );
        bar.formatOpt( false, "Hello World" );
        REQUIRE( bar == "Hello World" );
        bar.formatOpt( true, "ZZZ" );
        REQUIRE( bar == "Hello WorldZZZ" );
        bar = "bob";
        bar.format( nullptr );
        REQUIRE( bar == "bob" );
        bar.formatAppend( nullptr );
        REQUIRE( bar == "bob" );
        bar.formatOpt( true, nullptr );
        REQUIRE( bar == "bob" );

        vformat( bar, "Hello World" );
        REQUIRE( bar == "Hello World" );
        vformatAppend( bar, "XXX" );
        REQUIRE( bar == "Hello WorldXXX" );
        vformatOpt( false, bar, "Hello World" );
        REQUIRE( bar == "Hello World" );
        vformatOpt( true, bar, "ZZZ" );
        REQUIRE( bar == "Hello WorldZZZ" );
        bar = "bob";
        vformat( bar, nullptr );
        REQUIRE( bar == "bob" );
        vformatAppend( bar, nullptr );
        REQUIRE( bar == "bob" );
        vformatOpt( true, bar, nullptr );
        REQUIRE( bar == "bob" );
    }
    ...
    SECTION( "Compare..." )
    {
        FString<64> s1( "HELLO WORLD, THIS IS BOB" );
    ...
    SECTION( "iostream..." )
    {
        ...
        FString<128>  o1( "Hello6789ABCDEF" );
        out << o1;
        FString<128> o2( "World" );
        out << o2;
        out.close();

        std::ifstream in( "stream.out" );
        FString<15>   i1;  // 15 bytes length
        in >> i1;
        REQUIRE( i1 == o1 );
