#ifndef KIT_SYSTEM_SIM_TICK_H_
#define KIT_SYSTEM_SIM_TICK_H_
/*------------------------------------------------------------------------------
 ...
 */
#ifndef OPTION_KIT_SYSTEM_SIM_TICK_MIN_TICKS_FOR_ADVANCE
#define OPTION_KIT_SYSTEM_SIM_TICK_MIN_TICKS_FOR_ADVANCE 10
#endif

/** Value passed to the Kit::System::sleep() call to effectively yield the CPU
    do allow the threads running in simulated time to actually begin executing
 */
#ifndef OPTION_KIT_SYSTEM_SIM_TICK_YEILD_SLEEP_TIME
#define OPTION_KIT_SYSTEM_SIM_TICK_YEILD_SLEEP_TIME 1
#endif
...
/** This macro is a wrapper for the topLevelWait() call. The macro allows the
   ...
   */
#define KIT_SYSTEM_SIM_TICK_TOP_LEVEL_WAIT() Kit::System::SimTick::topLevelWait()
 ...
 */
#define KIT_SYSTEM_SIM_TICK_APPLICATION_WAIT() Kit::System::SimTick::applicationWait()
...
/** This macro is a wrapper for the usingSimTicks() call. The macro allows the call
  ...
  */
#define KIT_SYSTEM_SIM_TICK_USING_SIM_TICKS() Kit::System::SimTick::usingSimTicks()
 ...
 */
#define KIT_SYSTEM_SIM_TICK_THREAD_INIT_( f ) Kit::System::SimTick::threadInit_( f )
...
/** This COMPONENT Scoped macro is a wrapper for onThreadExit_() call.  The macro allows the
    ...
    application SHOULD NEVER use this macro
 */
#define KIT_SYSTEM_SIM_TICK_ON_THREAD_EXIT_() Kit::System::SimTick::onThreadExit_()


#else
/// Simulate Tick disabled
#define KIT_SYSTEM_SIM_TICK_TOP_LEVEL_WAIT()

/// Simulate Tick disabled
#define KIT_SYSTEM_SIM_TICK_APPLICATION_WAIT()

/// Simulate Tick disabled
#define KIT_SYSTEM_SIM_TICK_USING_SIM_TICKS() false

/// Simulate Tick disabled
#define KIT_SYSTEM_SIM_TICK_THREAD_INIT_( f )

/// Simulate Tick disabled
#define KIT_SYSTEM_SIM_TICK_REAL_TIME_THREAD_INIT_()

/// Simulate Tick disabled
#define KIT_SYSTEM_SIM_TICK_ON_THREAD_EXIT_()

#endif  // end USE_KIT_SYSTEM_SIM_TICK
...
<pre>
        ...
        o Model in the Loop (MIL).  An external application such as Matlab/
          ...
          provided by KIT as well as specialization to Matlib/Simulink
    ...
    NOTES:
    ------
        o All timing sources, e.g. Kit::System::sleep(), provided by KIT
          have support for simulated ticks build in.

        o All of the KIT's ITC message queues/mailbox servers, etc. also
          have support for simulated ticks build in.
        ...
        o With respect to threads that are using simulated time, There needs to
          ...
          a Kit::System::EventLoop (or a sub-class) instance - then this
          requirement has been met. Worse case you a create a thread that
          forever loops on sleep().

        o This interface is independent of the underlying platform, i.e works
          on ALL platforms.
        ...
        o NOT all threads and/or parts of an application need or should use
          ...
          not using simulated time.
            ...
            - The platform's native/main thread ALWAYS runs in 'real-time', i.e.
              call to SimTick::wait() never blocks.  Also any sleep() calls from
              the native/main thread are redirected to the Api::sleepInRealTime().
          ...
          non-simulated application.
            ...
            - Not all use cases using a simulated tick and threading will work
              ...
              period as completed in on pass of the "main loop".  Other issues:
                ...
                o The assumption is that all calls to a mutex will NOT cause the
    ...
    o Enabling Simulated time is done at compile done.  The following steps
      are required:
        1) Compile with the preprocessor symbol USE_KIT_SYSTEM_SIM_TICK
           defined.

        2) Compile & Link the src/Kit/System/_simtick directory.  Also do NOT
           compile/link the src/Kit/System/xxx/_realtime directories
    ...
    o Typically thats all that needs to be done. The only additional work is if
      ...
      a KIT Event Loop, mailbox server, etc.) 'forever' loop AND it is desired
      for the thread(s) to use 'simulated time'.  For this scenario the
      application must add a call to this interface at the top of the forever
      loop.  See the example below:
        ...
        void myThreadEntryFunction( void* arg )
            ...
            while(1)
                ...
                KIT_SYSTEM_SIM_TICK_TOP_LEVEL_WAIT();
                ...
                }
            }
         }
...
@endhtmlonly
 */
class SimTick : public Kit::Container::ListItem
{
public:
...
public:
     ...
     */
    static bool advance( uint64_t numTicks ) noexcept;
     ...
     */
    static bool isWaitingOnNextTick( KitSystemThreadID_T threadID ) noexcept;
...
protected:
    ...
    KitSystemThreadID_T m_threadId;
