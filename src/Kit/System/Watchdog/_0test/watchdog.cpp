...
TEST_CASE( "watchdog" )
{
    ...
    SECTION( "watchedeventthread NON supervisor mode" )
    {
        ...
        KIT_SYSTEM_WATCHDOG_START_EVENTLOOP( &nonSupervisorThread, timerManager );
        ...
        {
            sleep( 10 );
            KIT_SYSTEM_WATCHDOG_EVENTLOOP_MONITOR( &nonSupervisorThread );
        }

        REQUIRE( kickCount_ == kickCountBefore );

        KIT_SYSTEM_WATCHDOG_STOP_EVENTLOOP( &nonSupervisorThread );
    }
