...
void TimerManager::processTimers( void ) noexcept
{
    ...
    // Update my timers
    KIT_SYSTEM_TRACE_MSG( SECT_, " @@ START TICK: delta=%" PRIu32 ", now=%" PRIu32 ", [m_timeMark=%" PRIu32 "]", deltaTime, m_timeNow, m_timeMark );
    tick( deltaTime );
    ...
    while ( msec )
    {
        ...
        else
        {
            ...
            if ( msec <= nextCount )
            {
                ...
                KIT_SYSTEM_TRACE_MSG( SECT_, " @@   Time Decremented (msec=%" PRIu32 ", counter=%" PRIu32 ", %p)", msec, counterPtr->count(), counterPtr );
                msec = 0;
            }
            else
            {
                counterPtr->decrement( nextCount );
                KIT_SYSTEM_TRACE_MSG( SECT_, " @@   Time Decremented & Expired (msec=%" PRIu32 ", counter=%" PRIu32 ", %p)", msec, counterPtr->count(), counterPtr );
                msec -= nextCount;
            }
